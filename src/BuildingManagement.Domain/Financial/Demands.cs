...
public sealed class UnitReceivable : FinancialRecord
{
    ...
    public UnitReceivable(long unitAccountId, long fundId, long? allocationId, long? adjustmentId, decimal amount, string responsible, long? partyId, DateTimeOffset? dueDate, DateTimeOffset now) { if (allocationId.HasValue == adjustmentId.HasValue) throw new DomainValidationException("origin", "Exactly one origin is required."); Positive(amount); UnitAccountId = unitAccountId; FundAccountId = fundId; DemandAllocationId = allocationId; AccountAdjustmentId = adjustmentId; OriginalAmount = OutstandingAmount = amount; ResponsiblePartyTypeKey = responsible; ResponsiblePartyId = partyId; DueDate = dueDate; CreatedAtUtc = now; }
    public void ApplyPayment(decimal amount, DateTimeOffset now) { Positive(amount); if (amount > OutstandingAmount) throw new DomainValidationException("amount", "Cannot exceed outstanding amount."); OutstandingAmount -= amount; Status = OutstandingAmount == 0 ? FinancialKeys.ReceivableStatuses.Paid : FinancialKeys.ReceivableStatuses.PartiallyPaid; Touch(now); }
}

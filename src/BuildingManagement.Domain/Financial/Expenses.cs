...
public sealed class ExpenseDisbursement : Entity
{
    ...
    public ExpenseDisbursement(string code, long expenseId, long fundId, long? payeeId, decimal amount, string method, string? notes, DateTimeOffset now)
    { if (expenseId <= 0 || fundId <= 0 || amount <= 0) throw new DomainValidationException("amount", "Expense, fund, and positive amount are required."); Initialize(code, now); ExpenseId = expenseId; FundAccountId = fundId; PayeePartyId = payeeId; Amount = amount; PaymentMethodKey = Required(method, "paymentMethodKey"); Notes = Optional(notes); }
    public void Finalize(DateTimeOffset paidAt, DateTimeOffset now) { if (Status == FinancialKeys.Statuses.Finalized) return; if (Status != FinancialKeys.Statuses.Draft) throw new DomainValidationException("status", "Disbursement cannot be finalized."); Status = FinancialKeys.Statuses.Finalized; PaidAtUtc = paidAt.ToUniversalTime(); Touch(now); }
}

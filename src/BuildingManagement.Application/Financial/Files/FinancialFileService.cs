...
public sealed class FinancialFileService(IApplicationDbContext db, IFileStorage storage, FileStorageOptions options, TimeProvider clock, ILogger<FinancialFileService> logger) : FinancialServiceBase(db, clock)
{
    ...
    private async Task<FinancialFileResponse> Store(string ownerCode, string category, IncomingFile incoming, Func<StoredFile, Task> link, string? title, string? description, CancellationToken ct) { var valid = FileStoragePolicy.ValidateDocument(incoming, options); var name = $"{Guid.NewGuid():N}{valid.Extension}"; var segments = category.Split('/'); var key = segments.Length == 1 ? FileStoragePolicy.CreateStorageKey("financial", ownerCode, segments[0], name) : FileStoragePolicy.CreateStorageKey("financial", ownerCode, segments[0], segments[1], name); await storage.SaveAsync(new(key, incoming.Content), ct); var file = new StoredFile(await Unique(Db.StoredFiles, ct), valid.OriginalFileName, name, key, valid.ContentType, valid.Extension, valid.Length, null, StorageProviders.Local, Now); Db.StoredFiles.Add(file); try { await link(file); return new(new(file.Code, $"/api/v1/files/{file.Code}/content", file.OriginalFileName, file.ContentType, file.FileExtension, file.FileSizeBytes), title, description); } catch (Exception exception) { Db.Detach(file); try { await storage.DeleteAsync(key, ct); } catch (Exception cleanup) when (cleanup is not OperationCanceledException) { logger.LogError(cleanup, "Financial file cleanup failed for {StorageKey} after {ErrorType}.", key, exception.GetType().Name); } throw; } }
}

...
internal sealed class AccountRecoveryCaseConfiguration : IEntityTypeConfiguration<AccountRecoveryCase> { public void Configure(EntityTypeBuilder<AccountRecoveryCase> b) { ConfigurationHelpers.Entity(b, "AccountRecoveryCases"); b.Property(x => x.RecoveryTypeKey).HasColumnType("varchar(40)"); b.Property(x => x.StatusKey).HasColumnType("varchar(30)"); b.Property(x => x.Reason).HasMaxLength(1000); b.HasOne<User>().WithMany().HasForeignKey(x => x.UserId).OnDelete(DeleteBehavior.Restrict); b.HasOne<PlatformUser>().WithMany().HasForeignKey(x => x.ResolvedByPlatformUserId).OnDelete(DeleteBehavior.Restrict); } }

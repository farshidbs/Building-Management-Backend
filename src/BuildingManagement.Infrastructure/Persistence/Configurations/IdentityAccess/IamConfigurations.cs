...
{
    public void Configure(EntityTypeBuilder<AuthSession> b) { ConfigurationHelpers.Entity(b, "AuthSessions"); b.ToTable("AuthSessions", "bms", t => t.HasCheckConstraint("CK_AuthSessions_OneActor", "(CASE WHEN [UserId] IS NULL THEN 0 ELSE 1 END + CASE WHEN [PlatformUserId] IS NULL THEN 0 ELSE 1 END) = 1")); b.Property(x => x.ClientTypeKey).HasColumnType("varchar(30)"); b.Property(x => x.StatusKey).HasColumnType("varchar(30)"); b.Property(x => x.AccessTokenHash).HasMaxLength(128).IsRequired(); b.HasIndex(x => x.AccessTokenHash).IsUnique(); b.HasOne<User>().WithMany().HasForeignKey(x => x.UserId).OnDelete(DeleteBehavior.Restrict); b.HasOne<PlatformUser>().WithMany().HasForeignKey(x => x.PlatformUserId).OnDelete(DeleteBehavior.Restrict); b.HasOne<UserLoginMethod>().WithMany().HasForeignKey(x => x.AuthenticatedViaLoginMethodId).OnDelete(DeleteBehavior.Restrict); b.HasOne<Complex>().WithMany().HasForeignKey(x => x.ActiveComplexId).OnDelete(DeleteBehavior.Restrict); b.HasOne<Building>().WithMany().HasForeignKey(x => x.ActiveBuildingId).OnDelete(DeleteBehavior.Restrict); b.HasOne<AccessRole>().WithMany().HasForeignKey(x => x.ActiveRoleId).OnDelete(DeleteBehavior.Restrict); b.HasIndex(x => new { x.UserId, x.StatusKey }); }
}
internal sealed class AuthRefreshTokenConfiguration : IEntityTypeConfiguration<AuthRefreshToken>
{
    public void Configure(EntityTypeBuilder<AuthRefreshToken> b) { b.ToTable("AuthRefreshTokens", "bms"); b.HasKey(x => x.Id); b.Property(x => x.Id).UseIdentityColumn(); b.Property(x => x.TokenHash).HasMaxLength(128).IsRequired(); b.Property(x => x.RowVersion).IsRowVersion().IsConcurrencyToken(); b.HasIndex(x => x.TokenHash).IsUnique(); b.HasOne<AuthSession>().WithMany().HasForeignKey(x => x.AuthSessionId).OnDelete(DeleteBehavior.Restrict); b.HasOne<AuthRefreshToken>().WithMany().HasForeignKey(x => x.ReplacedByRefreshTokenId).OnDelete(DeleteBehavior.Restrict); }
}
...
internal sealed class AccessPermissionConfiguration : IEntityTypeConfiguration<AccessCapability>
{
    ...
    Seed(27, "asset_event_view", "مشاهده رویداد دارایی", "assets", 270, at), Seed(28, "asset_event_manage", "مدیریت رویداد دارایی", "assets", 280, at),
    Seed(29, "expense_view", "مشاهده هزینه", "finance", 290, at), Seed(30, "expense_create", "ایجاد هزینه", "finance", 300, at),
    Seed(31, "expense_update", "ویرایش هزینه", "finance", 310, at), Seed(32, "expense_finalize", "نهایی‌سازی هزینه", "finance", 320, at),
    Seed(33, "demand_view", "مشاهده مطالبه", "finance", 330, at), Seed(34, "demand_create", "ایجاد مطالبه", "finance", 340, at),
    Seed(35, "demand_update", "ویرایش مطالبه", "finance", 350, at), Seed(36, "demand_finalize", "نهایی‌سازی مطالبه", "finance", 360, at),
    Seed(37, "payment_view", "مشاهده پرداخت", "finance", 370, at), Seed(38, "payment_submit", "ثبت پرداخت", "finance", 380, at),
    Seed(39, "payment_confirm", "تأیید پرداخت", "finance", 390, at),
    Seed(40, "file_read", "مشاهده فایل", "files", 400, at), Seed(41, "file_read_confidential", "مشاهده فایل محرمانه", "files", 410, at),
    Seed(42, "membership_view", "مشاهده عضویت", "iam", 420, at), Seed(43, "membership_manage_scoped", "مدیریت عضویت", "iam", 430, at),
    Seed(44, "invitation_send", "ارسال دعوت", "iam", 440, at), Seed(45, "invitation_revoke", "لغو دعوت", "iam", 450, at),
    Seed(46, "access_grant_view", "مشاهده دسترسی تفویضی", "iam", 460, at), Seed(47, "access_grant_manage", "مدیریت دسترسی تفویضی", "iam", 470, at),
...
internal sealed class AccessRolePermissionConfiguration : IEntityTypeConfiguration<AccessRoleCapability>
{
    public void Configure(EntityTypeBuilder<AccessRoleCapability> b)
    {
        ...
        Add(4, 17, 19, 23, 25, 27, 29, 33, 37, 38, 40, 48, 51);
        Add(5, 17, 19, 23, 25, 27, 29, 33, 37, 40, 48);
        Add(8, 17, 19, 23, 25, 27, 29, 33, 37, 38, 40, 48, 51);
        b.HasData(rows);
    }
}
...
internal sealed class AccessGrantConfiguration : IEntityTypeConfiguration<AccessGrant> { public void Configure(EntityTypeBuilder<AccessGrant> b) { ConfigurationHelpers.Entity(b, "AccessGrants"); b.ToTable("AccessGrants", "bms", t => t.HasCheckConstraint("CK_AccessGrants_OneScope", "(CASE WHEN [ComplexId] IS NULL THEN 0 ELSE 1 END + CASE WHEN [BuildingId] IS NULL THEN 0 ELSE 1 END + CASE WHEN [UnitId] IS NULL THEN 0 ELSE 1 END) = 1")); b.Property(x => x.Reason).HasMaxLength(500); b.HasOne<User>().WithMany().HasForeignKey(x => x.UserId).OnDelete(DeleteBehavior.Restrict); b.HasOne<User>().WithMany().HasForeignKey(x => x.GrantedByUserId).OnDelete(DeleteBehavior.Restrict); b.HasOne<AccessCapability>().WithMany().HasForeignKey(x => x.PermissionId).OnDelete(DeleteBehavior.Restrict); b.HasOne<Complex>().WithMany().HasForeignKey(x => x.ComplexId).OnDelete(DeleteBehavior.Restrict); b.HasOne<Building>().WithMany().HasForeignKey(x => x.BuildingId).OnDelete(DeleteBehavior.Restrict); b.HasOne<Unit>().WithMany().HasForeignKey(x => x.UnitId).OnDelete(DeleteBehavior.Restrict); b.HasIndex(x => new { x.UserId, x.PermissionId, x.IsActive }); } }
internal sealed class BuildingAccessSettingConfiguration : IEntityTypeConfiguration<BuildingAccessSetting> { public void Configure(EntityTypeBuilder<BuildingAccessSetting> b) { ConfigurationHelpers.Entity(b, "BuildingAccessSettings"); b.Property(x => x.UnitVisibilityKey).HasColumnType("varchar(30)"); b.Property(x => x.FinancialVisibilityKey).HasColumnType("varchar(30)"); b.HasOne<Building>().WithMany().HasForeignKey(x => x.BuildingId).OnDelete(DeleteBehavior.Restrict); b.HasIndex(x => x.BuildingId).IsUnique(); } }
internal sealed class InvitationConfiguration : IEntityTypeConfiguration<Invitation> { public void Configure(EntityTypeBuilder<Invitation> b) { ConfigurationHelpers.Entity(b, "Invitations"); b.ToTable("Invitations", "bms", t => t.HasCheckConstraint("CK_Invitations_OneScope", "(CASE WHEN [ComplexId] IS NULL THEN 0 ELSE 1 END + CASE WHEN [BuildingId] IS NULL THEN 0 ELSE 1 END + CASE WHEN [UnitId] IS NULL THEN 0 ELSE 1 END) = 1")); b.Property(x => x.TokenHash).HasMaxLength(128); b.Property(x => x.NormalizedIdentifierValue).HasMaxLength(200); b.Property(x => x.InvitationTypeKey).HasColumnType("varchar(30)"); b.HasOne<AccessRole>().WithMany().HasForeignKey(x => x.RoleId).OnDelete(DeleteBehavior.Restrict); b.HasOne<User>().WithMany().HasForeignKey(x => x.InvitedByUserId).OnDelete(DeleteBehavior.Restrict); b.HasOne<User>().WithMany().HasForeignKey(x => x.AcceptedByUserId).OnDelete(DeleteBehavior.Restrict); b.HasOne<Complex>().WithMany().HasForeignKey(x => x.ComplexId).OnDelete(DeleteBehavior.Restrict); b.HasOne<Building>().WithMany().HasForeignKey(x => x.BuildingId).OnDelete(DeleteBehavior.Restrict); b.HasOne<Unit>().WithMany().HasForeignKey(x => x.UnitId).OnDelete(DeleteBehavior.Restrict); b.HasOne<UnitPartyRelation>().WithMany().HasForeignKey(x => x.SourceUnitPartyRelationId).OnDelete(DeleteBehavior.Restrict); b.HasIndex(x => x.TokenHash).IsUnique(); b.HasIndex(x => new { x.NormalizedIdentifierValue, x.RoleId, x.ComplexId, x.BuildingId, x.UnitId }).IsUnique().HasFilter("[AcceptedAtUtc] IS NULL AND [RevokedAtUtc] IS NULL AND [IsActive] = 1"); } }
internal sealed class MembershipExitRequestConfiguration : IEntityTypeConfiguration<MembershipExitRequest> { public void Configure(EntityTypeBuilder<MembershipExitRequest> b) { ConfigurationHelpers.Entity(b, "MembershipExitRequests"); b.Property(x => x.StatusKey).HasColumnType("varchar(30)"); b.Property(x => x.Reason).HasMaxLength(500); b.HasOne<AccessMembership>().WithMany().HasForeignKey(x => x.MembershipId).OnDelete(DeleteBehavior.Restrict); b.HasOne<User>().WithMany().HasForeignKey(x => x.RequestedByUserId).OnDelete(DeleteBehavior.Restrict); b.HasOne<User>().WithMany().HasForeignKey(x => x.DecidedByUserId).OnDelete(DeleteBehavior.Restrict); } }
internal sealed class IdentityConflictReviewConfiguration : IEntityTypeConfiguration<IdentityConflictReview> { public void Configure(EntityTypeBuilder<IdentityConflictReview> b) { ConfigurationHelpers.Entity(b, "IdentityConflictReviews"); b.Property(x => x.ConflictTypeKey).HasColumnType("varchar(50)"); b.Property(x => x.StatusKey).HasColumnType("varchar(30)"); b.Property(x => x.Details).HasMaxLength(1000); b.HasOne<User>().WithMany().HasForeignKey(x => x.UserId).OnDelete(DeleteBehavior.Restrict); b.HasOne<Party>().WithMany().HasForeignKey(x => x.PartyId).OnDelete(DeleteBehavior.Restrict); } }
internal sealed class AccountRecoveryCaseConfiguration : IEntityTypeConfiguration<AccountRecoveryCase> { public void Configure(EntityTypeBuilder<AccountRecoveryCase> b) { ConfigurationHelpers.Entity(b, "AccountRecoveryCases"); b.Property(x => x.ReferenceHash).HasMaxLength(128).IsRequired(); b.Property(x => x.RecoveryTypeKey).HasColumnType("varchar(40)"); b.Property(x => x.StatusKey).HasColumnType("varchar(30)"); b.Property(x => x.OldNormalizedIdentifier).HasMaxLength(200); b.Property(x => x.NewNormalizedIdentifier).HasMaxLength(200); b.Property(x => x.IdentityNumberEvidence).HasMaxLength(50); b.Property(x => x.Reason).HasMaxLength(1000); b.HasIndex(x => x.ReferenceHash).IsUnique(); b.HasOne<User>().WithMany().HasForeignKey(x => x.UserId).OnDelete(DeleteBehavior.Restrict); b.HasOne<UserLoginMethod>().WithMany().HasForeignKey(x => x.OldLoginMethodId).OnDelete(DeleteBehavior.Restrict); b.HasOne<PlatformUser>().WithMany().HasForeignKey(x => x.ResolvedByPlatformUserId).OnDelete(DeleteBehavior.Restrict); } }
...
internal sealed class PlatformRolePermissionConfiguration : IEntityTypeConfiguration<PlatformRolePermissionLink> { public void Configure(EntityTypeBuilder<PlatformRolePermissionLink> b) { b.ToTable("PlatformRolePermissions", "bms"); b.HasKey(x => x.Id); b.Property(x => x.Id).UseIdentityColumn(); b.HasOne<PlatformRole>().WithMany().HasForeignKey(x => x.RoleId).OnDelete(DeleteBehavior.Restrict); b.HasOne<PlatformCapability>().WithMany().HasForeignKey(x => x.PermissionId).OnDelete(DeleteBehavior.Restrict); b.HasIndex(x => new { x.RoleId, x.PermissionId }).IsUnique(); } }
internal sealed class PlatformUserRoleConfiguration : IEntityTypeConfiguration<PlatformUserRoleLink> { public void Configure(EntityTypeBuilder<PlatformUserRoleLink> b) { b.ToTable("PlatformUserRoles", "bms"); b.HasKey(x => x.Id); b.Property(x => x.Id).UseIdentityColumn(); b.HasOne<PlatformUser>().WithMany().HasForeignKey(x => x.PlatformUserId).OnDelete(DeleteBehavior.Restrict); b.HasOne<PlatformRole>().WithMany().HasForeignKey(x => x.RoleId).OnDelete(DeleteBehavior.Restrict); b.HasIndex(x => new { x.PlatformUserId, x.RoleId }).IsUnique(); } }
internal sealed class SupportActingSessionConfiguration : IEntityTypeConfiguration<SupportActingSession> { public void Configure(EntityTypeBuilder<SupportActingSession> b) { ConfigurationHelpers.Entity(b, "SupportActingSessions"); b.Property(x => x.Reason).HasMaxLength(1000); b.HasOne<PlatformUser>().WithMany().HasForeignKey(x => x.PlatformUserId).OnDelete(DeleteBehavior.Restrict); b.HasOne<User>().WithMany().HasForeignKey(x => x.TargetUserId).OnDelete(DeleteBehavior.Restrict); } }
internal sealed class SecurityAuditEventConfiguration : IEntityTypeConfiguration<SecurityAuditEvent> { public void Configure(EntityTypeBuilder<SecurityAuditEvent> b) { b.ToTable("SecurityAuditEvents", "bms"); b.HasKey(x => x.Id); b.Property(x => x.Id).UseIdentityColumn(); b.Property(x => x.EventTypeKey).HasColumnType("varchar(80)"); b.Property(x => x.ResourceKindKey).HasColumnType("varchar(50)"); b.Property(x => x.ResourceCode).HasMaxLength(100); b.Property(x => x.Reason).HasMaxLength(1000); b.Property(x => x.IpAddress).HasMaxLength(64); b.HasOne<User>().WithMany().HasForeignKey(x => x.UserId).OnDelete(DeleteBehavior.Restrict); b.HasOne<PlatformUser>().WithMany().HasForeignKey(x => x.PlatformUserId).OnDelete(DeleteBehavior.Restrict); b.HasOne<AuthSession>().WithMany().HasForeignKey(x => x.AuthSessionId).OnDelete(DeleteBehavior.Restrict); b.HasIndex(x => new { x.UserId, x.OccurredAtUtc }); } }
internal sealed class BuildingRolePermissionOverrideConfiguration : IEntityTypeConfiguration<BuildingRolePermissionOverride> { public void Configure(EntityTypeBuilder<BuildingRolePermissionOverride> b) { ConfigurationHelpers.Entity(b, "BuildingRolePermissionOverrides"); b.Property(x => x.EffectKey).HasColumnType("varchar(10)"); b.HasOne<Building>().WithMany().HasForeignKey(x => x.BuildingId).OnDelete(DeleteBehavior.Restrict); b.HasOne<AccessRole>().WithMany().HasForeignKey(x => x.RoleId).OnDelete(DeleteBehavior.Restrict); b.HasOne<AccessCapability>().WithMany().HasForeignKey(x => x.PermissionId).OnDelete(DeleteBehavior.Restrict); b.HasIndex(x => new { x.BuildingId, x.RoleId, x.PermissionId }).IsUnique(); } }

...
The new Iranian mobile is proven with the dedicated `account_recovery_new_mobile` OTP purpose. Recovery OTP delivery keeps the one-minute cooldown and the shared limit of five OTP requests per normalized mobile across all purposes within ten minutes. Each challenge is bound to the recovery case and normalized new mobile, so login, registration, invitation, and LoginMethod-add proofs cannot be replayed. Candidate resolution uses old LoginMethod history plus exact optional identity evidence through the primary UserPartyLink; it never uses fuzzy names or arbitrary PartyContact values. Ambiguous evidence creates an IdentityConflictReview and never silently selects or merges an identity. A live recovery in `pending_mobile_verification`, `pending_review`, or `approved` may be cancelled through `POST /api/v1/auth/recovery/{reference}/cancel`; cancellation is terminal and preserves LoginMethods, Sessions, User, and Party state.
...
The production OTP provider remains intentionally unconfigured until an SMS vendor is chosen. In the Development environment only, `DevelopmentConsoleOtpDelivery` prints the mobile and plaintext OTP to the API console for local Swagger testing. Production always registers the fail-closed unconfigured provider; never enable console OTP outside Development. Invitation, LoginMethod/Session lifecycle, account recovery, Platform/support acting, Membership exit, and individual AccessGrant workflows are implemented. Context-selection UX, notification delivery, broad audit analytics, Platform credential recovery, and SSO remain deferred.
